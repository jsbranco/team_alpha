Meteor.methods({
    'followUser': function (userId) {
        ...
        if (Meteor.user().following.indexOf(userId) >= 0) {
            throw new Meteor.Error("already-following", "Already following this user");
        }

        Meteor.users.update({_id: Meteor.user()._id}, {$push: {"following": userId}}, function (err, doc) {
            if (err)
    ...
    'unfollowUser': function (userId) {
        ...
        Meteor.users.update({_id: Meteor.user()._id}, {$pull: {"following": userId}}, function (err, doc) {
            if (err)
                throw new Meteor.Error("error-updating", err.message);
